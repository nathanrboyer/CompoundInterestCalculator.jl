...
# ╔═╡ 253c7db5-6d95-4c3d-ab19-2f4e106b8dc2
md"Enter values for each of the variables below, including a guess for the one you will solve for."

# ╔═╡ 692b42fa-eb31-41ef-bb8d-36c08b58b9f4
md"# Solution"

# ╔═╡ 12e51311-f0ba-4503-b942-c064bbd6630c
md"# Compound Interest Equation"
...
"""
	eqsolve(eq, vars, solvefor) -> finalvars

Solve the provided `eq` equation for the variable `solvefor`.
`vars` should include all variables in the equation `eq`,
including a guess for the `solvefor` variable.
Output will be a copy of `vars` with the `solvefor` variable value replaced.
All inputs must be defined using Symbolics.jl `@variables`.

# Arguments
- `eq::Equation`: a Symbolics.jl equation definition
- `vars::Dict{Num, <:Real}`: a dictionary of variable values
- `solvefor::Num`: the variable name to solve for

# Returns
- `finalvars::Dict{Num, <:Real}`: a dictionary of variable values including the computed solution
"""
function eqsolve(
	eq::Equation,
	vars::Dict{Num, <:Real},
	...
	guess = vars[solvefor]
	fixedvars = filter(p -> !isequal(p.first, solvefor), vars)
	@named ns = NonlinearSystem([eq], [solvefor], keys(fixedvars))
	nscomplete = complete(ns)
	prob = NonlinearProblem(nscomplete, [guess], fixedvars)
	...
	finalvars = convert(Dict{Num, typeof(sol)}, vars)
	finalvars[solvefor] = sol
	return finalvars
end

# ╔═╡ 4b24c253-c50a-4e69-a0d6-313fb58dfbd4
finalvars = eqsolve(eq, vars, solvefor);

# ╔═╡ 2355ebad-3817-4131-8759-f5ff2d23e52f
md"# Helper Functions"

# ╔═╡ e01125fb-ef27-47b3-a73b-377cea0ce7ff
"Creates a callable function from the right hand side of equation `eq`."
eqfunction = build_function(eq.rhs, P, A, r, n, t) |> eval

# ╔═╡ 0f6db8f9-30e1-4f1f-9534-aec65e99a044
"Closure of `eqfunction` over time `t`."
value_over_time(t) = eqfunction(finalvars[P], finalvars[A], finalvars[r], finalvars[n], t) 

# ╔═╡ f6820291-8b18-4a13-8848-ce00b8cf7b52
begin
	plot(
		value_over_time,
		range(0, finalvars[t], round(Int, finalvars[t]*finalvars[n])+1),
		thickness_scaling = 1.4,
		linewidth = 3,
		yformatter = x -> "\$" * format(x, commas=true),
		legend = false,
		title = "Fund Value Over Time",
		xlabel = "Time (years)",
		ylabel = "Value (\$)",
	)
	local fives = 5:5:round(Int, finalvars[t])
	scatter!(
		fives,
		value_over_time.(fives),
		markercolor = :black,
		markersize = 2,
		series_annotations = [text(
			"\$" * format(round(Int,value_over_time(i)), commas=true),
			6,
			:right,
			:bottom,
		) for i in fives],
	...
	annotate!(
		(0.5, 0.85),
		text(
			"$solvefor = $(format(finalvars[solvefor], commas=true))",
			theme_palette(:auto)[3],
		),
	)
end

# ╔═╡ 37771c3f-2b9d-444d-8ed4-d175b9a2bd83
md"# Packages"
...
git-tree-sha1 = "befc86364d9b36abc6d874f91ab5d7b4d5e6bb62"
...
git-tree-sha1 = "aa9dbbc7e5c8b240bc22f434364ab4eae2b9e2a8"
